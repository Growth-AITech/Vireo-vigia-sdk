...
- [x] `src/vireo_vigia/__init__.py` — version export
- [x] `src/vireo_vigia/config.py` — Pydantic Settings fail-fast (SecretStr, model_validator)
- [x] `src/vireo_vigia/logging.py` — structlog JSON/console, bind_context, get_logger
- [x] `src/vireo_vigia/exceptions.py` — 13-type hierarchy (VireoVigiaError → LLMError → RateLimitError, etc.)
- [x] `src/vireo_vigia/cli.py` — typer entrypoint (version, ingest stub, chat stub)
...
- [x] Discord bot `Vireo Agent` live in server
...
- [ ] (Optional) Telegram bot via @BotFather → VIREO_TELEGRAM_BOT_TOKEN
