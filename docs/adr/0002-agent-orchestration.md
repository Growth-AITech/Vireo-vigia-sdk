...
Non-`VireoVigiaError` exceptions in `_get_rag_context` and `_get_chain_context`
are caught, logged as warnings, and replaced with empty context:

```python
except VireoVigiaError:
...
knowledge without RAG context.  `VireoVigiaError` subtypes (e.g.
