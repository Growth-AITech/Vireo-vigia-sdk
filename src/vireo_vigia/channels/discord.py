...
Connects the Vireo Vigía to a Discord server via slash commands.
Uses ``discord.py`` (``pip install 'vireo-vigia[discord]'``).
...
Setup:
  ...
  2. Add a Bot, copy the token → set VIREO_DISCORD_BOT_TOKEN in .env
...
from vireo_vigia.exceptions import VireoVigiaError
...
class DiscordChannelAdapter:
    """
    Discord bot adapter wrapping the Vireo Vigía.
    ...
    async def start(self) -> None:
        ...
        @tree.command(
            name="ask",
            description="Ask the Vireo AI agent anything about DeFi, positions, or protocol docs.",
            guild=guild_obj,
        )
        ...
        @tree.command(
            name="help",
            description="List Vireo Vigía commands.",
            guild=guild_obj,
        )
        async def help_cmd(interaction: Any) -> None:  # noqa: ANN401
            text = (
                "**Vireo Vigía** — DeFi AI assistant\n\n"
                ...
                "`/help` — this message\n\n"
                "*Powered by Vireo · [vireo-vigia](https://github.com/Growth-AITech/Vireo-vigia-sdk)*"
            )
        ...
        async def monitor_cmd(
            ...
            from vireo_vigia.monitoring.health_monitor import HealthMonitor
...
def create_adapter_from_settings(settings: Any, agent: Agent) -> DiscordChannelAdapter:  # noqa: ANN401
    """Factory: build a ``DiscordChannelAdapter`` from ``VireoSettings``."""
    if not settings.discord_bot_token:
        raise ValueError(
            "VIREO_DISCORD_BOT_TOKEN is not set. "
