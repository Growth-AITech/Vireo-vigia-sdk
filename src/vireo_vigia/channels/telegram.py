...
Connects the Vireo Vigía to Telegram via the Bot API.
Uses ``python-telegram-bot`` (``pip install 'vireo-vigia[telegram]'``).
...
Setup:
  ...
  2. Set VIREO_TELEGRAM_BOT_TOKEN in .env
...
class TelegramChannelAdapter:
    """
    Telegram bot adapter wrapping the Vireo Vigía.
...
def create_adapter_from_settings(settings: Any, agent: Agent) -> TelegramChannelAdapter:  # noqa: ANN401
    """Factory: build a ``TelegramChannelAdapter`` from ``VireoSettings``."""
    if not settings.telegram_bot_token:
        raise ValueError("VIREO_TELEGRAM_BOT_TOKEN is not set")
