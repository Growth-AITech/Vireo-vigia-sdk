"""CLI entrypoint — ``vireo-vigia`` command."""
...
app = typer.Typer(
    name="vireo_vigia",
    help="Vireo Vigía SDK — deploy AI agents with on-chain context.",
...
def version() -> None:
    ...
    from vireo_vigia import __version__

    typer.echo(f"vireo-vigia v{__version__}")
...
def ingest(
    ...
    collection: str = typer.Option(
        "vireo_docs", "--collection", "-c", help="Qdrant collection name"
    ),
) -> None:
    """
    ...
    Examples:
      ...
      vireo-vigia ingest https://docs.yourprotocol.io/overview

      vireo-vigia ingest ./whitepaper.md --title "Protocol Whitepaper"
    """
    ...
    async def _run() -> None:
        ...
        from vireo_vigia.knowledge.embeddings import create_from_settings
        from vireo_vigia.knowledge.ingest import IngestPipeline, SemanticChunker
        from vireo_vigia.knowledge.models import Document
...
def chat(
    ...
    collection: str = typer.Option(
        "vireo_docs", "--collection", "-c", help="Qdrant collection for RAG"
    ),
    ...
    """
    Start an interactive chat session with the Vireo Vigía.
    ...
    Examples:
      ...
      vireo-vigia chat 0xYOUR_WALLET --protocol "Aave"
    """
    ...
    async def _run() -> None:
        ...
        async with chain_reader:
            while True:
                ...
                console.print()
                console.print(Rule("[bold cyan]Vireo Vigía[/bold cyan]", style="cyan"))
                ...
                try:
                    ...
                    if "Connection" in err_type or "connect" in str(exc).lower():
                        ...
                        console.print("[dim]  → Check VIREO_ANTHROPIC_API_KEY in .env[/dim]")
                    continue
