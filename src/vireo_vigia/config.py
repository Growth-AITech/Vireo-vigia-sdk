...
All config is loaded from environment variables (prefixed VIREO_) or a .env
...
class VireoSettings(BaseSettings):
    """
    Application settings loaded from environment variables.

    All variables are prefixed with ``VIREO_``.
    Example: ``VIREO_ANTHROPIC_API_KEY=sk-ant-...``
    """
    ...
    openai_api_key: SecretStr | None = Field(
        default=None,
        description="OpenAI API key — only needed if VIREO_EMBEDDING_PROVIDER=openai",
    )
    ...
    qdrant_collection: str = Field(default="vireo_docs")
...
def load_settings() -> VireoSettings:
    """
