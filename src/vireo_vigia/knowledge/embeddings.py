...
``OpenAIEmbeddings``
    ...
    Requires ``VIREO_OPENAI_API_KEY``.

Switch backend via ``VIREO_EMBEDDING_PROVIDER=openai`` (default: ``local``).
"""
...
from vireo_vigia.exceptions import AuthenticationError, EmbeddingError
...
class OpenAIEmbeddings:
    ...
    Requires ``VIREO_OPENAI_API_KEY``.
    Only needed if ``VIREO_EMBEDDING_PROVIDER=openai``.
...
def create_from_settings(settings: Any) -> EmbeddingProvider:  # noqa: ANN401
    """
    Build the appropriate embedding backend from ``VireoSettings``.

    Uses ``LocalEmbeddings`` by default.
    Set ``VIREO_EMBEDDING_PROVIDER=openai`` to use OpenAI instead.
    """
    provider = getattr(settings, "embedding_provider", "local")
    if provider == "openai":
        if not settings.openai_api_key:
            raise EmbeddingError(
                "VIREO_OPENAI_API_KEY is required when VIREO_EMBEDDING_PROVIDER=openai",
