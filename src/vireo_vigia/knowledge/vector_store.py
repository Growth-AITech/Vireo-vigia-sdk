...
collection lifecycle management, and Vireo Vigía exception mapping.
"""
...
from vireo_vigia.exceptions import VectorStoreError
from vireo_vigia.knowledge.models import Chunk, SearchResult
...
class QdrantVectorStore:
    ...
    def __init__(
        ...
        try:
            ...
            if url == ":memory:":
                self._client: Any = AsyncQdrantClient(":memory:")
            else:
...
def create_from_settings(settings: Any) -> QdrantVectorStore:  # noqa: ANN401
    """Factory: build ``QdrantVectorStore`` from a ``VireoSettings`` instance."""
