...
- Maps Anthropic SDK exceptions → Vireo Vigía typed exceptions
"""
...
from vireo_vigia.exceptions import AuthenticationError, ContextLengthError, LLMError, RateLimitError
...
def create_from_settings(settings: Any) -> AnthropicLLM:  # noqa: ANN401
    """
    Factory: build an ``AnthropicLLM`` from a ``VireoSettings`` instance.

    Avoids importing ``VireoSettings`` directly to prevent circular imports.
    """
    return AnthropicLLM(
