...
from vireo_vigia.intelligence.alerts import AlertCondition, AlertEngine
from vireo_vigia.intelligence.digest import DigestReport, PortfolioDigest
from vireo_vigia.intelligence.whale import WhaleTracker
...
class TestPortfolioDigest:
    async def test_generate_empty_portfolio(self) -> None:
        ...
        chain.get_user_balance = AsyncMock(
            return_value={"account_value": 10000.0, "health_factor": 999.0}
        )
        ...
        llm.chat = AsyncMock(
            return_value=LLMResponse(
                text="No positions. Portfolio is in cash.",
    ...
    async def test_generate_with_positions(self) -> None:
        chain = AsyncMock()
        chain.get_user_balance = AsyncMock(
            return_value={"account_value": 50000.0, "health_factor": 1.8}
        )
        chain.get_user_positions = AsyncMock(
            return_value=[
                {
